...
#Start default Performance collector
Start-SMPerformanceCollector -CollectorName 'Server Manager Performance Monitor'
Write-Output "SMPerformanceCollector is $(Get-SMPerformanceCollector -CollectorName 'Server Manager Performance Monitor')"

#Reconfigure Services
$Services = 'ualsvc','dssvc'
foreach($Service in $Services){
    & Sc.exe config $Service type=own
    Get-Service -Name $Service | Restart-Service -Force
    Get-Service -Name $Service | select ServiceName,ServiceType,Starttype
}
